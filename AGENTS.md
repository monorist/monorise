...
| **IaC** | SST v4 (Serverless Stack) for AWS infrastructure |
...
│   ├── proxy/         # @monorise/proxy — Helpers for server-side proxy layers (Next.js API routes etc.)
│   ├── sst/           # @monorise/sst — SST v4 module (infrastructure components)
...
react → depends on base
  ↓
sst → depends on SST v4 (peer)
  ↓
proxy → depends on base
  ↓
...
1. Copies `dist` from all sub-packages into `packages/monorise/dist/{base,core,react,sst,cli,proxy}/`
...
### `@monorise/proxy` — Server-Side Proxy Helpers

**Files:**
- `index.ts` — Main exports

Helpers for a server-side proxy layer (Next.js API routes and similar).
Exports `generateWebSocketTicket()`, which mints a short-lived, one-time
WebSocket ticket so a browser client can open an authenticated socket
without holding a long-lived credential. Reads `API_BASE_URL` from the
environment and throws if it is unset.
...
- `components/analytics.ts` — Athena/Glue analytics resources
...
npx @biomejs/biome check --write .  # Fix issues
