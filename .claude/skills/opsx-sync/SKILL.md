---
name: opsx-sync
description: Sync a change's delta specs into the main specs using /skill:opsx-sync command. Use when the user types /skill:opsx-sync to update main specs from a change without archiving it.
---
...
**Input**: Optionally specify a change name after `/skill:opsx-sync` (e.g., `/skill:opsx-sync add-auth`). If omitted, check if it can be inferred from conversation context. If vague or ambiguous you MUST prompt for available changes.
...
1. **Select the change**
   ...
   Always announce: "Using change: <name>" and how to override (e.g., `/skill:opsx-sync <other>`).
