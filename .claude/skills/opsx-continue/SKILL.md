---
name: opsx-continue
description: Continue working on an OpenSpec change by creating its next artifact using /skill:opsx-continue command. Use when the user types /skill:opsx-continue to progress a change or create the next artifact.
---
...
**Input**: Optionally specify a change name after `/skill:opsx-continue` (e.g., `/skill:opsx-continue add-auth`). If omitted, check if it can be inferred from conversation context. If vague or ambiguous you MUST prompt for available changes.
...
1. **Select the change**
   ...
   Always announce: "Using change: <name>" and how to override (e.g., `/skill:opsx-continue <other>`).
...
3. **Act based on status**:
   ...
   **If all planning artifacts are complete (`isPlanningComplete: true`, or legacy `isComplete: true`)**:
   ...
   - Suggest: "Planning is complete! You can now implement this change with `/skill:opsx-apply`. Once implementation and any tracked work are complete, archive it with `/skill:opsx-archive`."
   - STOP
...
- Prompt: "Run `/skill:opsx-continue` to create the next artifact"
